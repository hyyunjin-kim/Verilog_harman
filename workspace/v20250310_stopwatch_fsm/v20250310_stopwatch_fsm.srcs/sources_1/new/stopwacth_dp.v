...
module stopwacth_dp(
    ...
    time_counter #(.TICK_COUNT(100), .BIT_WIDTH(7)) U_Time_Msec(
        .clk(clk), .reset(reset), .tick(w_clk_100hz), .o_time(msec), .o_tick(w_msec_tick)
    );
    time_counter #(.TICK_COUNT(60), .BIT_WIDTH(7)) U_Time_Sec(
        .clk(clk), .reset(reset), .tick(w_msec_tick), .o_time(sec), .o_tick(w_sec_tick)
    );
    time_counter #(.TICK_COUNT(60), .BIT_WIDTH(7)) U_Time_Min(
        .clk(clk), .reset(reset), .tick(w_sec_tick), .o_time(min), .o_tick(w_min_tick)
    );
    time_counter #(.TICK_COUNT(24), .BIT_WIDTH(5)) U_Time_Hour(
        .clk(clk), .reset(reset), .tick(w_min_tick), .o_time(hour), .o_tick()
    );
