`timescale 1ns / 1ps

// 수정 중간

module top_fifo_uart(
    input clk, rst,
    input rx,
    input wr,
    input [7:0] wr_data,
    output [7:0] rd_data,
    //output rd,
    output tx,
    output w_rx_empty
    );

    wire [7:0] w_tx_rdata, w_rx_data;  // wire인지 reg인지 판단 필요
    wire w_tx_empty, w_tx_done;
    wire w_tx_full;
    // wire w_rx_empty;
 
    // assign tx_rd = ~w_tx_done & ~w_tx_empty;
    // assign rx_rd = ~w_rx_empty;

    fifo fifo_tx (
     .clk(clk),
     .rst(rst),
    // write
     .wdata(wr_data), //이제 거리 수치 받아올 곳
     .wr(wr), //wr 여러번 받아올 거 생각할 곳
     .full(w_tx_full),  //full 더이상 어떤 의미가 있는지 궁금
    // read
     .rd(~w_tx_done), // w_tx_done 대신 TX FIFO가 비어 있지 않을 때 읽기
     .rdata(w_tx_rdata),
     .empty(w_tx_empty)
    );

    fifo fifo_rx (
     .clk(clk),
     .rst(rst),
    // write
     .wdata(w_rx_data),
     .wr(w_rx_done),
     .full(),   // full 핀 연결
    // read
     .rd(~w_tx_full),  //초음파센서에서 이제 실행? 신호? 보내도 돼요~ 신호호
     .rdata(rd_data), //버튼 의미 전송용
     .empty(w_rx_empty)  //empty가 이제 의미 있나 궁금
    );

    uart U_uart (
    .clk(clk),
    .reset(rst),
    .btn_start(~w_tx_empty),
    .tx_data_in(w_tx_rdata),
    .tx(tx),
    .tx_done(w_tx_done),
    .rx(rx),
    .rx_done(w_rx_done),
    .rx_data(w_rx_data)
    );
endmodule
