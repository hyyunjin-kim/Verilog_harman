...
//    counter_9999 cnt99(.clk(w_slow_clk), .reset(w_clear), .clear(w_clear), .count_stop(w_run_stop), .count(count_value));
    counter_tick ct100(.clk(clk), .reset(reset), .clear(w_clear), .tick(w_tick_100hz), .counter(w_tick_msec));//,.o_tick(w_tick_msec));
    counter_tick #(.TICK_COUNT(6000)) ct60(.clk(clk), .reset(reset), .clear(w_clear), .tick(w_tick_100hz), .counter(w_sec_count));//,.o_tick(w_sec_count));
    fnd_controller fc1(.clk(clk), .reset(reset), .bcd(w_tick_msec), .bcd_sec_count(w_sec_count), .seg(seg), .seg_comm(seg_comm));
...
// module clk_divider_slow (   //클럭을 받아 느린 클럭으로 바꾸는 모듈듈
//     input clk, reset,
//     output reg slow_clk
// );
...
module counter_tick  #(parameter TICK_COUNT = 100) (
    input clk, reset,
    input tick,
    input clear,
